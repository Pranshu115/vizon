// Script to upload Tata 709g LPT PDF reports to Supabase Storage
...
const TRUCK_FOLDER_NAME = 'TATA_709G_LPT/REPORTS'
const SOURCE_FOLDER = path.join(__dirname, 'Tata 709g LPT')

// Report files to upload
const REPORT_FILES = [
  'Legal Report DL1LAF9060.pdf',
  'VEHICLE INSPECTION REPORT DL1LAF9060.pdf'
]
...
async function uploadPDFToSupabase(filePath, fileName) {
  try {
    ...
    const uniqueFileName = `${TRUCK_FOLDER_NAME}/${timestamp}-${baseName.replace(/\s+/g, '_')}${fileExtension}`
...
async function uploadAllReports() {
  console.log('🚀 Starting to upload Tata 709g LPT reports to Supabase...\n')
  ...
  // Save mapping file
  const mappingFile = path.join(__dirname, 'tata-709g-lpt-reports-mapping.json')
  ...
  console.log('      cp tata-709g-lpt-reports-mapping.json public/')
