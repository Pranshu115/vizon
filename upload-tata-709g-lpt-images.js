// Script to upload all Tata 709g LPT images and video to Supabase Storage
...
const TRUCK_FOLDER_NAME = 'TATA_709G_LPT' // Storage folder name in Supabase
...
const findSourceFolder = () => {
    ...
    const dirs = fs.readdirSync(__dirname).filter(f => {
      try {
        return fs.statSync(path.join(__dirname, f)).isDirectory() && 
               f.trim().toLowerCase() === 'tata 709g lpt'
      } catch {
        return false
      }
    })
  ...
  return path.join(__dirname, 'Tata 709g LPT')
}
...
async function updateTruckWithImages(imageUrls) {
  try {
    // Get the truck ID - using exact name "Tata 709g LPT"
    const { data: truck, error: truckError } = await supabase
      .from('trucks')
      .select('id')
      .eq('name', 'Tata 709g LPT')
      .single()
...
async function uploadAllMedia() {
  console.log('🚀 Starting to upload Tata 709g LPT images and video to Supabase...\n')
  ...
  if (!fs.existsSync(SOURCE_FOLDER)) {
    ...
    console.error(`   Please make sure the folder "Tata 709g LPT" exists in the project root`)
    process.exit(1)
  }
