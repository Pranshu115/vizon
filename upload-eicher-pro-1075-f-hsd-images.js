// Script to upload all Eicher Pro 1075 F HSD images to Supabase Storage
...
const TRUCK_FOLDER_NAME = 'EICHER_PRO_1075_F_HSD' // Storage folder name in Supabase
...
const findSourceFolder = () => {
    ...
    const dirs = fs.readdirSync(__dirname).filter(f => {
      try {
        return fs.statSync(path.join(__dirname, f)).isDirectory() && 
               f.trim().toLowerCase() === 'eicher pro 1075 f hsd'
      } catch {
        return false
      }
    })
  ...
  return path.join(__dirname, 'EICHER PRO 1075 F HSD ')
}
...
async function updateTruckWithImages(imageUrls) {
  try {
    // Get the truck ID - try different name variations
    const nameVariations = [
      'EICHER PRO 1075 F HSD',
      'Eicher Pro 1075 F HSD',
      'Eicher Motors Pro 1075 F HSD',
      'Eicher 1075 F HSD'
    ]
    ...
    console.log(`\n✅ Updated truck (ID: ${truck.id}, Name: ${truckName}) with main image`)
...
async function uploadAllMedia() {
  console.log('🚀 Starting to upload Eicher Pro 1075 F HSD images to Supabase...\n')
  ...
  if (!fs.existsSync(SOURCE_FOLDER)) {
    ...
    console.error(`   Please make sure the folder "EICHER PRO 1075 F HSD" exists in the project root`)
    process.exit(1)
  }
  ...
  // Save mapping file
  const mappingFile = path.join(__dirname, 'eicher-pro-1075-f-hsd-image-mapping.json')
