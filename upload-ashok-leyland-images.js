// Script to upload all ASHOK LEYLAND ECOMET STAR 1415 HE images to Supabase Storage
...
const TRUCK_FOLDER_NAME = 'ASHOK_LEYLAND_ECOMET_STAR_1415_HE'
const SOURCE_FOLDER = path.join(__dirname, 'ASHOK LEYLAND ECOMET STAR 1415 HE')
...
async function updateTruckWithImages(imageUrls) {
  try {
    ...
    // Store all images in a JSON format (we'll need to add a column or use existing structure)
    // For now, update the main image_url
...
async function uploadAllImages() {
  console.log('🚀 Starting to upload ASHOK LEYLAND ECOMET STAR 1415 HE images to Supabase...\n')
  ...
  // Save mapping file
  const mappingFile = path.join(__dirname, 'ashok-leyland-image-mapping.json')
