// Script to upload Eicher Pro 1075 F HSD reports to Supabase
...
const SOURCE_FOLDER = path.join(__dirname, 'EICHER PRO 1075 F HSD ')
const BUCKET_NAME = 'truck-images'
const TRUCK_FOLDER_NAME = 'EICHER_PRO_1075_F_HSD/REPORTS'
const REPORT_FILES = [
  'Copy of Legal Report DL1LAB9450.pdf',
  'Copy of VEHICLE INSPECTION REPORT DL1LAB9450.pdf'
]
...
async function updateTruckWithReports(reports) {
  try {
    ...
    console.log('✅ Successfully updated truck with report URLs')
    if (legalReport) {
...
async function uploadAllReports() {
  console.log('🚀 Starting to upload Eicher Pro 1075 F HSD reports to Supabase...\n')
  ...
  // Save mapping file
  const mappingFile = path.join(__dirname, 'eicher-pro-1075-f-hsd-reports-mapping.json')
  ...
  console.log('      cp eicher-pro-1075-f-hsd-reports-mapping.json public/')
